...
* “Folder” is a logical concept—actual container is a **bucket**.
...
* **Bucket naming**
    ...
    * Should not use "aws" names or trademarks (like `aws-bucket123`)—AWS discourages it.
...
* Before making anything public, first disable “Block Public Access” at the bucket level.
  * Go to **S3 → Bucket → Permissions → Block Public Access**
  * Turn OFF “Block all public access” (only if you really want public access)
  * Save changes

* Use **Bucket Policy** to allow public read access for objects.
  * Go to **Permissions → Bucket Policy → Policy Generator**
  * Select:
...
* To verify:
  * Open any object → Copy object URL
  * Paste into browser → Should load without authentication
...
* S3 has **no real folder structure**
  ...
  * “Folders” are only a visual representation
  ...
  * Example key: `images/2024/user/profile.png`
    ...
    * `2024/` → sub-prefix
    * `user/` → sub-prefix
    * `profile.png` → actual file name part of key
...
* Object key MUST be unique inside bucket
  ...
  * If you upload another file with same key → it replaces the previous object (unless versioning enabled)

* Object key structure creates “prefixes”
...
* Each part is uploaded independently:
  ...
  * Supports parallel upload → faster performance

* Multipart upload steps:

  * **Step 1** – Initiate upload → S3 returns `UploadId`
  * **Step 2** – Upload individual parts using `UploadId`
  * **Step 3** – Complete upload → parts are combined into final object
  * (Optional) Abort upload → deletes temporary parts
...
* Advantages:
  ...
  * Can retry failed parts independently → reduces upload failure
...
* Parallel upload example:
  ...
  * Part 1: Bytes 0–5MB
  * Part 2: Bytes 5MB–10MB
  * Part 3: Bytes 10MB–15MB
...
* Important internal concepts:
  ...
  * Final ETag looks like:
    ...
    * `abcd1234-10` → where `10` indicates number of parts
...
* After enabling versioning:
  * Enable → bucket becomes versioned permanently.
...
* Versioning states:
  ...
  * **Disabled** – default state (no version IDs).
  * **Enabled** – full versioning.
  * **Suspended** – versioning paused; new uploads get "null" version.

* Version ID:

  * Automatically assigned by AWS.
  * Uniquely identifies each version of an object.
  * For suspended buckets → new version = "null"
...
* How versioning impacts S3 storage:
  ...
  * If you upload a 10 MB file 10 times → total storage = 100 MB.
...
* A null version ID is assigned to objects when versioning is disabled or suspended on an S3 bucket.
    ```bash
    Versioning disabled → upload file → file gets null version
    Enable versioning → upload file → file gets version A
    Upload again → version B
    Suspend versioning → upload again → overwrite null version
    ```
...
* Old versions remain → cost increases if many deletes are made.
...
* **Permanently delete**
    * In a versioned bucket, a normal delete only creates a delete marker — it does NOT permanently delete data.
    * To permanently delete, you must delete every version of the object—including delete markers.
    ```bash
...
* Access control options:
  * Public website → requires public-read objects
  * Private website → use CloudFront signed URLs or IAM policies
...
* DNS using Route 53:
  ...
  * OR use S3 website endpoint (A record → alias)
...
* For `example.com`
    * Add **A record (Alias)** → points to S3 website endpoint of `example.com`
* For `www.example.com`
    * Add **A record (Alias)** → points to S3 website endpoint of `www.example.com`
...
* **S3 Redirection Rules?**
    ...
    * They allow S3 to:
        * Redirect prefix → another prefix
        * Redirect specific file → another file
    ...
    * **Common Use Cases + Examples**
        ...
        * Redirect Single Page to Another Page
            * Example: `/about.html` → `/about-us.html`
        ...
        * Redirect Old Prefix to External URL
            * Example: `/blog/*` → `https://newsite.com/blog/`
        ...
        * Redirect Based on HTTP Error Code (Fallback Redirect)
            * Example: if 404 → redirect to `/index.html` (good for SPA apps)
        ...
        * Prefix-Based Conditional Redirect With Hostname Change
            * Example: `/docs/*` → `https://newdomain.com/help/*`
...
```
```js
// ✅ 2. Redirect to a Specific Protocol
// Forces redirect from http → https or vice-versa
{
  "Redirect": { "Protocol": "https" }
}
```
```js
// ✅ 3. Replace Key Prefix (prefix-to-prefix rewrite)
...
```
```js
// ✅ 4. Replace Key With (redirect to a single file)
...
```
```js
// ✅ 5. Set HTTP Redirect Code (301 or 302)
...
```
```js
// ✅ 8. Key Prefix + ReplaceKeyPrefixWith
...
```
```js
// ✅ 10. Key Prefix + HostName (redirect to subdomain)
...
```
```js
// ✅ 12. Key Prefix + HttpRedirectCode
...
```
```js
// ✅ 13. Error Code + ReplaceKeyPrefixWith
// 404 fallback → rewrite prefix
{
  "Condition": { "HttpErrorCodeReturnedEquals": "404" },
...
```
```js
// ✅ 14. Error Code + ReplaceKeyWith
// 404 fallback → redirect to specific file (SPA routing)
{
  "Condition": { "HttpErrorCodeReturnedEquals": "404" },
...
```
```js
// ✅ 15. Error Code + HostName
// If file missing → send to backup domain
{
  "Condition": { "HttpErrorCodeReturnedEquals": "404" },
...
```
```js
// ✅ 16. Error Code + Protocol
// If file missing → force https version
{
  "Condition": { "HttpErrorCodeReturnedEquals": "404" },
  "Redirect": { "Protocol": "https" }
}
```
```js
// ✅ 17. Error Code + HttpRedirectCode
// If file missing → use specified redirect code
{
  "Condition": { "HttpErrorCodeReturnedEquals": "404" },
...
```
```js
// ✅ 18. Redirect Without Any Condition
...
```
```js
// ✅ 19. Redirect to different host + replace prefix
...
```
```js
// ✅ 20. Redirect to different host + replace full key
...
#### S3 Transfer Acceleration
  ...
  * A feature that accelerates uploads/downloads to an S3 bucket using Amazon CloudFront’s global edge network.
  * Edge Location used ---> CDN, S3 Transfer Acceleration.
  * Speeds up transfer for long-distance uploads (e.g., India → US bucket).
  * Uses AWS edge locations to route the upload to nearest AWS region over optimized network path.
  * Requirements / Conditions
    * Bucket name cannot contain periods if you want SSL endpoint (ex: mybucket.mydomain.com → ❌).
      ```bash
  ...
  * How to Enable S3 Transfer Acceleration – Console (GUI)
    ...
    * Choose:
      * ✅ Enabled
      * (Optional) Suspended → disable but keep configuration
    * Save changes
  * Activation takes ~10–20 minutes to propagate.

  * How to Enable S3 Transfer Acceleration – AWS CLI
    ```bash
  ...
  * Accelerated Endpoint URL
    ...
    ```
  * If bucket name contains dots → SSL certificate mismatch → use HTTP or virtual style.
  * How to Use Accelerated Transfer
    ...
    * Upload using Accelerated endpoint (curl)
      ...
      That’s why S3 returns AccessDenied.
  ...
  * Pricing (Important)
    * You pay extra only for accelerated transfers:
      * ✅ Uploads/downloads via accelerated endpoint
      * ❌ Normal endpoint remains standard price

  * Common Mistakes & Solutions
    * ❌ Accelerated endpoint not working
    * ➡️ Solution: Wait 10–20 minutes after enabling
    <br>
    * ❌ SSL warning
    * ➡️ Bucket name has dots (e.g., mahin.raza.site) → use HTTP or rename bucket
    <br>
    * ❌ Upload failing with 403
    * ➡️ You must allow s3:PutObject permission for uploader identity
    <br>
    * ❌ No speed improvement
    * ➡️ Acceleration helps only when client is far from bucket region
  ...
  * Terraform – Enable S3 Transfer Acceleration
    ```hcl
  ...
  * CloudFormation – S3 Transfer Acceleration YAML
    ```yaml
    Resources:
      MyBucket:
        Type: AWS::S3::Bucket
...
#### addressing_style
  ...
  * Virtual Hosted–Style Addressing (Recommended by AWS)
    ```bash
...
* Solution
    ```
    # 1️⃣ Use path-style addressing (no wildcard issue)
    #     https://s3.amazonaws.com/my.bucket.name/object
    # 2️⃣ Rename bucket (BEST practice)
    #     mybucketname
    #     my-bucket-name
    # 3️⃣ Put S3 behind CloudFront + your own ACM certificate
    #     - custom domain works
    #     - SSL fully valid
...
* **S3 Same Region Replication (SRR)**
    ...
    * It replicates objects from source bucket → destination bucket within the SAME AWS region.
    * Example:
    ...
    * **It is asynchronous replication (not instant, usually sec–min)**
        * "Asynchronous" = not immediate, not real-time, not blocking.
            * Time can vary:
                * Same region: ~0.1s–5s
                * Cross region: ~2s–30s (can go higher under heavy load)
    ...
    * **Internal Flow**
      * Client uploads object → S3 stores it and returns HTTP 200 OK.
      ...
      * Destination copy takes seconds → sometimes minutes.
    ...
    * **How SRR Works (Internal Workflow)**
        ...
        * Replication engine scans rule → checks prefix/tag filters → decides if object qualifies.
    ...
    * **SRR Use Cases (Very Detailed)**
        ...
        * Log Aggregation
            * Multiple application buckets → one central logging bucket.
    ...
    * Not cross-region → can’t survive a full regional outage.
    ...
    * **How to Configure SRR (Same-Region Replication)**
      * Example:
      ...
      ```bash
      aws s3api create-bucket --bucket mahinraza-1 --region ap-south-1
      aws s3api create-bucket --bucket mahinraza-2 --region ap-south-1
      ```
      ...
      * Step 2: Create Replication Rule
        * Using AWS Console (easy method):
            * Go to S3 → my-source-bucket
            * Open Management tab
            * Click Replication
            * Click Add rule
            * Choose Entire bucket or Prefix filter
            * Choose Destination bucket → my-destination-bucket (same region)
            * Select:
...
* **Cross Region Replication (CRR)**
    ...
    * **How to Configure CRR (Cross-Region Replication)**
      * Example:
        * Source bucket: `source-logs-ap → Region = ap-south-1`
        * Destination bucket: `backup-logs-eu → Region = eu-west-1`
      ...
      * Step 2: Add Bucket Policy on Destination Bucket
        ...
        * Example (replace bucket names + account ID):
            ...
            ```
            * If SRR (same account), you don’t need cross-account policy.
      ...
      * Step 4: Configure Replication Rule in Source Bucket
        * S3 → source-logs-ap → Management → Replication → Add rule
        * Destination Bucket: backup-logs-eu
        * Choose “Cross-region replication”
...
* **Cross-Account Replication (CRR) between two AWS accounts (Account A → Account B)**
  ...
  * **How Cross-Account Replication Works (Simple Flow)**
    ...
    * Destination bucket policy allows Account A’s replication role to write objects.
  ...
  * **Required Permissions (Simple Explanation)**
    ...
    * Account B grants Account A’s replication role permission to:
  ...
  * Step 2: Add Bucket Policy on Destination Bucket (Account B)
    * This policy allows Account A’s replication role to write objects.
    * Replace:
    ...
    * Step 4: Configure Replication Rule in Source Bucket (Account A)
      * Using console:
        * `Go to S3 → source-bucket → Management → Replication → Add rule`
        * Choose:
...
* **Why Versioning Is Required for Replication?**
  ...
  * Without version IDs → S3 cannot track which object change needs to be replicated.
...
* **Delete Marker Replication**
  ...
  * If disabled:
    * The destination bucket will keep the file, even if deleted in source → acts like a backup.
  * Important:
...
* **Replica Modification Sync**
  ...
  * Example:
    * If you change metadata in the source after replication:
      * Without this option → destination object metadata does NOT update.
      * With this option → destination metadata updates to match source.
  * What metadata?
    * ACLs
    * Tags
...
### S3 Server Access Logging
  ...
  * How to Enable S3 Logging (GUI Method)
    * Step 1 — Create/Choose a Dedicated Destination Bucket for Logs
    * Step 2 — Go to Source Bucket
  ...
  * Destination bucket must allow source bucket to write logs.
    ...
    ```json
    {
      ...
      "Statement": [
        {
          "Sid": "S3PolicyStmt-DO-NOT-MODIFY-1581501376244",
          "Effect": "Allow",
          "Principal": {
            "Service": "logging.s3.amazonaws.com"
          },
          "Action": "s3:PutObject",
          "Resource": "arn:aws:s3:::youtube-s3-content-log/*"
        }
      ]
    }
    ```
...
| ------------------------------ | ---------------- | ------------ | ---------------------- | ------------ | -------------- | -------------- | ---------------- | ----------------------------------- |
| **STANDARD**                   | Frequent         | 99.99%       | 99.999999999% (11 9’s) | High         | Low            | ms             | None             | Websites, apps, high traffic data   |
| **STANDARD-IA**                | Infrequent       | 99.9%        | 11 9’s                 | Lower        | Higher         | ms             | 30 days          | Backups, DR, rarely accessed data   |
| **ONE ZONE-IA**                | Infrequent       | 99.5%        | 11 9’s                 | Lower        | Higher         | ms             | 30 days          | Non-critical, recreateable data     |
| **GLACIER Instant Retrieval**  | Rare/Infrequent  | 99.9%        | 11 9’s                 | Low          | Higher         | ms             | 90 days          | Archive with fast access            |
| **GLACIER Flexible Retrieval** | Rare             | 99.99%       | 11 9’s                 | Very Low     | High           | Minutes–Hours  | 90 days          | Long-term archive, compliance       |
| **GLACIER Deep Archive**       | Very Rare        | 99.99%       | 11 9’s                 | Lowest       | Highest        | Hours–Days     | 180 days         | Long-term preservation (7–10 years) |

<br>

##### **Storage Class – Detailed Explanation**
...
##### **Access Frequency – Detailed Explanation**

* Defines **how often you plan to access the data** stored in that class.
* Classes are optimized based on expected usage:
  * Frequent access → High-speed access → Higher storage cost
  * Infrequent access → Cheaper storage → Higher retrieval cost
  * Rare/Very rare access → Deep archive → Very cheap storage → Very slow retrieval
...
##### **Durability – Detailed Explanation**
...
##### **Storage Cost – Detailed Explanation**
...
##### **Retrieval Cost – Detailed Explanation**
...
##### **Retrieval Time – Detailed Explanation**
...
**Minimum Storage Days – Detailed Explanation**
...
* If you delete the object early → AWS still bills full 30/90/180 days.
...
* Glacier Deep Archive:
  * Long-term regulatory data retention (7–10 years or more)
...
* **Change Storage Class After Upload (via Lifecycle Rule)**
  * AWS doesn’t allow directly changing storage class of an existing object; but you can do it with a Lifecycle Rule.
...
* **Supported Storage Class Transitions**
  * Higher class to lower class only
    ...
    * Standard-IA → Glacier classes
    * Glacier Instant → Glacier Flexible / Deep Archive (rare)
...
* **Lifecycle Rule Actions**
  * Lifecycle rules in Amazon S3 support five major actions. These actions automatically manage objects based on time, age, prefix, or tags.
    * Transition current versions of objects between storage classesn → Move to cheaper storage class
    * Transition noncurrent versions of objects between storage classes
    * Expire current versions of objects → Delete old versions
    * Permanently delete noncurrent versions of objects
    * Delete marker removal → Remove unnecessary delete markers
    * Abort multipart uploads → Clean incomplete uploads
  
  * **Transition Actions**
    * Transition = Move objects to a different storage class after a specific number of days.
    * Automatically changes object storage class from expensive → cheaper.
  ...
  * **Delete Marker Removal**
    ...
    * If current version is a delete marker → lifecycle can remove it
...
| ------------------------------------- | ------------------------------------------ | ------------------------------------ | ---------------------------- | ----------------------------------------------- | ------------------------------------------ |
| **Log Archive**                       | Keep logs in Standard for 30 days          | After 30 days → Standard-IA          | After 90 days → Glacier      | After 180 days → Delete                         | Reduce cost of old logs and auto-clean     |
| **Backup Management**                 | Store daily DB backups in Standard         | After 7 days → IA                    | After 90 days → Deep Archive | Delete after 1 year                             | Manage backup lifecycle + cost reduction   |
| **Version Control Cleanup**           | Store latest version in Standard           | Older versions transition not needed | —                            | Delete old versions + remove delete markers     | Reduce storage cost in versioned buckets   |
| **Cost Optimization for Big Buckets** | Keep current objects in Standard initially | Move cold objects to IA              | —                            | Clean incomplete multipart uploads after 7 days | Optimize storage + remove unused fragments |
...
* **Object Size" Filter**
  ...
  * **Real-Life Use Cases**
    ...
    | ------------------------------------------------- | ----------------------- | ------------------------------- | ----------------------------------------------------------------------- | ----------------------------------------------------------------- |
    | **Avoid IA charges on small objects**             | Minimum size            | **128 KB**                      | Lifecycle applies only to objects ≥128 KB                               | Small objects stay in Standard → No unnecessary IA billing        |
    | **Avoid Glacier retrieval delays for huge files** | Maximum size            | **1 GB**                        | Lifecycle applies only to objects ≤1 GB                                 | Large files stay in IA/Standard → Medium-size files go to Glacier |
...
* Ideal for unpredictable access patterns where you don’t know how often a file will be accessed.
* How It Works
  * S3 automatically monitors object access.
  * If object is frequently accessed → stays in frequent tier.
  * If object becomes inactive → moves to infrequent tier.
  * If object is not accessed for long → moves to archive tiers (optional).
...
* No minimum storage duration
  * Unlike Standard-IA, Glacier, Deep Archive…
  * Intelligent-Tiering has NO minimum retention charge.
  * You can delete object any time → no penalty.
...
* **Benefits**
  ...
  * Fast automatic transitions—no performance impact.
  * PUT/GET performance unchanged, same as S3 Standard.
  * High throughput—handles millions of objects efficiently with no throttling.
...
| ------------------------------- | ------------- | --------------- | ------------------------------------------------------ | --------------------------------------------------------------- |
| **Frequent Access Tier**        | ✅ FREE        | ⚡ Milliseconds  | New/recently accessed object                           | You can download anytime without extra cost                     |
| **Infrequent Access (IA) Tier** | ✅ FREE        | ⚡ Milliseconds  | After object is unused for ~30 days                    | No extra download charges even if object becomes cold           |
| **Archive Instant Access Tier** | ❌ Costs money | ⚡ Milliseconds  | After long inactivity (Intelligent-Tiering auto-moves) | Retrieval has *Glacier-like cost* even though access is instant |
| **Archive Access Tier**         | ❌ Costs money | ⏳ Minutes–Hours | After extended inactivity                              | Slow + charged retrieval similar to Glacier retrieval           |
| **Deep Archive Access Tier**    | ❌ Costs money | 🕒 Hours–Days   | After very long inactivity                             | Cheapest storage but most expensive/slow retrieval              |
...
* By default:
  ...
  * Example:
    Website running on `https://mywebsite.com` tries to load file from `https://my-bucket.s3.amazonaws.com` → BLOCKED without CORS.
...
* **Common Scenarios Where CORS Is Required**
  ...
  * CloudFront → S3 origin with custom headers
  * Web app fetching JSON files from S3

* **How CORS Works (Simplified Flow)**
  * Browser sends a **request** to S3
  * S3 checks bucket’s **CORS rules**
  * If allowed origin → S3 sends **Access-Control-Allow-Origin** header
  * Browser allows the response
  * If not allowed → Browser blocks access (403 CORS error)
...
* **Detailed Explanation of Each Element**
  * **AllowedOrigins**
    ...
    * Examples:
      * `"*"` → allow all domains
      * `"https://myapp.com"` → allow specific website
      * `"https://dev.myapp.com"` → allow only dev environment
...
* **Use Cases**
  ...
  * Allow GET Only (Public Read / CDN / Static Content)
    ...
    * **Risk:** Public GET allowed – use for public website buckets only
  ...
  * Allow Specific Website Full Access (Most Common Production Rule)
    ...
    * **Risk:** Only one trusted domain can access → Safe
...
* **How Presigned URL Works (Internal Flow)**
  ...
  * Client (browser or mobile app) uses the URL directly → no AWS credentials required
  * AWS S3 validates:
    * Signature correctness
    * Expiration time
    * HTTP method allowed (GET/PUT)
  * If valid → access is granted
  * Once expired → URL stops working automatically
...
* **Generate a Pre-Signed URL Using AWS CLI**
  * For Download (GET)
    ...
    * You can share this URL — anyone can download that object without AWS credentials until it expires.
  
  * For Upload (PUT)
    * You can’t directly presign a PUT URL via `aws s3 presign`,
    * so use the AWS SDK (Python, Node.js, etc.) — see below.
...
* **Security Tips**
  * Set **short expiration** (5–15 mins) for uploads.
  * Use **IAM policies** to restrict who can generate URLs.
  * Don’t expose your AWS keys publicly — only share the pre-signed link.
  * Ensure your **bucket policy doesn’t block HTTPS** (since pre-signed URLs use HTTPS).
...
| Upload (PUT)   | Python `generate_presigned_url('put_object', …)`   | Temporary upload link          |
...
* **Why You Should Restrict URL Generation**
* Pre-signed URLs grant temporary but real access to S3 objects.
* Anyone who can call s3:PutObject or s3:GetObject on a bucket can generate URLs.
* If every user can do that, they can:
  * Upload files outside your workflow
  * Expose private data
  * Create links for outsiders → So only trusted roles (like backend servers) should generate them.
* Allow Only Backend Role to Generate Pre-Signed URLs:
  ```json
  {
    "Version": "2012-10-17",
    "Statement": [
      {
        "Sid": "AllowOnlyBackendRoleToAccessObjects",
        "Effect": "Allow",
        "Principal": {
          "AWS": "arn:aws:iam::111122223333:role/backend-app-role"
        },
        "Action": [
          "s3:GetObject",
          "s3:PutObject"
  ...
  ```json
  {
    ...
    "Statement": [
      {
        "Sid": "DenyUnauthorizedPresigners",
        "Effect": "Deny",
        "Action": ["s3:GetObject", "s3:PutObject"],
        "Resource": "arn:aws:s3:::my-secure-bucket/*",
        "Condition": {
          "StringNotEquals": {
            "aws:PrincipalArn": "arn:aws:iam::111122223333:role/backend-app-role"
          }
        },
        "Principal": "*"
      }
  ...
  ```
  * Only your backend role can perform GetObject / PutObject → and thus can generate pre-signed URLs.
  * All others are denied (even if they try via AWS SDK or CLI).
* Attach this IAM policy only to your backend app role or user that generates URLs:
  ```json
  {
    "Version": "2012-10-17",
    "Statement": [
      {
        "Sid": "AllowPresignActions",
        "Effect": "Allow",
        "Action": [
          "s3:GetObject",
          "s3:PutObject"
...
* **Accessing S3 Objects – HTTP vs HTTPS**\
...
* **Server-Side-Encryption**
  ...
  **a) SSE-S3 (AES-256) – Amazon Managed Keys**
  ...
  **b) SSE-KMS – AWS Key Management Service (KMS Keys)**
...
* **Example Scenario Explained (from transcript)**
  ...
  * Solution → Enable S3 Bucket Key to reduce KMS calls and cost significantly

* **How Encryption Works (Deep Internal Flow)**
  * Upload Flow (Example: SSE-KMS)
    1. Client uploads object → includes header `"x-amz-server-side-encryption: aws:kms"`.
...
* **Encryption + Replication (Important)**
  * **SSE-S3** → Works perfectly with SRR/CRR
  * **SSE-KMS** → Requires permissions on both source & destination keys
...
* This policy blocks all HTTP access for all objects in the bucket.
  ...
  ```
  * `aws:SecureTransport = false` → The policy denies the request came over HTTP.
  * Browser shows:
    * “Access Denied”
    * OR “403 Forbidden”
...
### Use Amazon CloudFront with Custom SSL Certificate
* If you must keep dots in the bucket name (e.g., for a domain-style bucket like mahin.raza.site):
* Use CloudFront as a secure HTTPS proxy in front of your bucket.
* Steps:
  * Go to CloudFront Console
  * Create a new distribution
  * Set Origin domain = your bucket endpoint
    e.g. mahin.raza.site.s3.amazonaws.com
  * In Viewer Protocol Policy → select “Redirect HTTP to HTTPS”
  * In SSL Certificate, select:
    * Custom SSL Certificate (ACM) if you own a domain
      e.g., *.mahin.raza.site
  * Deploy → CloudFront gives you a secure HTTPS domain
    e.g., https://d123abc.cloudfront.net

✅ Works securely even with dots in bucket name
✅ No SSL warning
✅ Industry-standard practice for hosting websites